...
const OrdersPage = () => {
			...
			try {
				...
				if (clientResponse.status === 200) {
					...
					const ordersResponse = await axios.get(
						`https://cpbackend.azurewebsites.net/api/orders/client/${clientResponse.data.id}`,
						{ withCredentials: true }
					);
							...
							const orderItemsWithDetails = await Promise.all(
								order.orderItems.map(async (item, index) => {
									const productResponse = await axios.get(
										`https://cpbackend.azurewebsites.net/api/products/${item.productId}`
									);
									return {
										...item,
	...
	const handleSaveItem = async (orderId, itemTempId) => {
		...
		try {
			...
			await axios.put(
				`https://cpbackend.azurewebsites.net/api/orders/${orderId}`,
				orderToSend,
				{ withCredentials: true }
			);
